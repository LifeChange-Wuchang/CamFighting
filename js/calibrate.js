...
function loop() {
  ...
  $('readout').innerHTML =
    '<span class="cal-swatch" style="background:' + rgb + '"></span>' +
    '<span class="big">' + rgb + '</span><br>' +
...
function hueStats(hs) {
  ...
  if (mean < 0) mean += 360;
  const dev = hs.map(h => { const d = Math.abs(h - mean); return Math.min(d, 360 - d); });
  return { mean, maxDev: Math.max(...dev) };
}
function inHueRange(h, lo, hi) {
  return lo <= hi ? (h >= lo && h <= hi) : (h >= lo || h <= hi);
}
function toRanges(lo, hi) {
  lo = (lo + 360) % 360; hi = (hi + 360) % 360;
  return lo <= hi ? [[Math.round(lo), Math.round(hi)]]
                  : [[0, Math.round(hi)], [Math.round(lo), 360]];
}
...
function analyse() {
  ...
  let lo = st.mean - st.maxDev - 10;
  let hi = st.mean + st.maxDev + 10;
  let satMin = Math.max(0, Math.min(...good.map(s => s.s)) - 0.07);
  let valMin = Math.max(0, Math.min(...good.map(s => s.v)) - 0.07);

  const hits = () => bad.filter(s =>
    inHueRange(s.h, (lo + 360) % 360, (hi + 360) % 360) && s.s >= satMin && s.v >= valMin);
  ...
  return {
    ...
    sat: Number(satMin.toFixed(2)),
    val: Number(valMin.toFixed(2)),
    leaks: leaks.length,
    tightened,
    hueMean: st.mean,
    spread: st.maxDev
...
$('btnClear').onclick = () => {
  ...
  $('nGood').textContent = '0'; $('nBad').textContent = '0';
};
...
$('btnFinish').onclick = () => {
  ...
  const swatches = arr => '<div class="samples">' + arr.map(s =>
    '<i style="background:rgb(' + Math.round(s.r) + ',' + Math.round(s.g) + ',' + Math.round(s.b) + ')"></i>'
  ).join('') + '</div>';

  const note = a.leaks > 0
    ? '<div class="warn"><b>⚠️ 有 ' + a.leaks + ' 筆皮膚/背景樣本無法排除。</b><br>' +
      '這代表頭巾顏色和皮膚或環境太接近,再怎麼調門檻都會誤判。<br>' +
  ...
  const tips = a.tightened
    ? '<p class="sub" style="text-align:left">為了排除皮膚/背景,門檻有被自動收緊,判定會比較嚴格。' +
      '若現場覺得太難命中,可以把飽和度下限再調低 0.03 試試。</p>'
    : '';

  const hueText = a.ranges.map(r => r[0] + '-' + r[1]).join(',');

  box.innerHTML =
    '<h2 style="font-size:19px;margin:0 0 4px">校色結果</h2>' +
    '<p class="sub" style="text-align:left">平均色相 ' + a.hueMean.toFixed(0) +
      '°,樣本間最大偏差 ±' + a.spread.toFixed(0) + '°</p>' +
    ...
    note + tips +
    '<p class="sub" style="text-align:left">把下面三個數值填進<b>主持人控台 → 隊伍設定</b>對應的那一隊:</p>' +
    '<pre id="snippet">色相範圍  ' + hueText + '\n飽和度下限  ' + a.sat + '\n亮度下限  ' + a.val + '</pre>' +
  ...
  $('btnCopy').onclick = async () => {
    try {
      ...
      $('btnCopy').textContent = '請長按上方文字手動複製';
    }
  };
};
